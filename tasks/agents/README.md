...
| 3 | **D — Story Progression** | [`lot_D_story_progression.md`](lot_D_story_progression.md) | A, B (idéal) | DONE | Helper `bot/world_navigation.change_location` partagé pipeline MOVE texte libre + `/move` deprecated ; persiste Campaign + recharge `session.npcs` ; `GameSession.advance_beat_if_ready` (fuzzy difflib seuil 0.7) hooké en fin de `_continue_from_resolution` + `StoryArcRepository.update` ; embed `bot/embeds/beat_embed.py` posté après le scene embed. Tests : `tests/test_world_navigation.py` (3) + `tests/scenarios/test_beat_advance.py` (6) verts. |
...
| 5 | **E — Trivial NPC Death** | [`lot_E_trivial_npc_death.md`](lot_E_trivial_npc_death.md) | **C** | DONE | `trivial_resolve` (1 jet, dégâts, mort) + propagation : `NPC.kill`, retrait `location.npcs_present`, world-fact markdown, `story_bible.log_event`, témoins FRIENDLY/ALLIED → HOSTILE. Persistance via `db_factory` repos. Tests : combat (4) + scénario trivial kill + log_event (2) verts ; ruff/mypy clean sur fichiers touchés. |
